...
namespace RealEstate_Dapper_UI.ViewComponents.Dashboard
{
    public class _DashboardLastFourContactListComponentPartial : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:44300/api/Contact/GetLastFourContactAsync");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<ResultLastFourContactDto>>(jsonData);
                return View(values);
            }
            return View();
        }
